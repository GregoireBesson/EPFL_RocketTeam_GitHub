...
static const uint16_t CRC_16_TABLE[] PROGMEM = {
...
// See https://users.ece.cmu.edu/~koopman/crc/index.html for good polynomials
    static constexpr SimpleCRC::Parameters CRC_16_GENERATOR_POLY = {0xA2EB, 0xFFFF, 0xFFFF, false, false};

    static uint16_t CalculateRemainderFromTable(uint8_t byte, uint16_t remainder);

    static uint16_t Finalize(uint16_t remainder);
...
*/
uint16_t SimpleCRC::CalculateRemainderFromTable(const uint8_t byte, uint16_t remainder) {
    remainder = (remainder << 8) ^
                pgm_read_word(CRC_16_TABLE + ((remainder >> 8) ^ byte));
...
*/
uint16_t SimpleCRC::Finalize(uint16_t remainder) {
