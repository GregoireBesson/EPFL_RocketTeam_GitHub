...
% flightdata = [t, acc_x, acc_y, acc_z, gyro_x, gyro_y, gyro_z];
% t = t -40;
% indStart = find(t>0,1);
% indEnd = find(t>7.42,1);
% flightdata = flightdata(indStart:indEnd,:);

% plot(flightdata(:,1)-40,flightdata(:,3))
% xlabel('t(s)')
% ylabel('Acceleration roll axis(m/s/s)')
%%
%%
% Gyro calibration
gyro_bias_t = 35;
...
indEnd = find(t>7.32,1);
flightdata = flightdata(indStart:indEnd,:);
%%
t=flightdata(:,1)-40;
roll_acel = flightdata(:,3)-gMag;
...
indStart=find(t>4,1)
indEnd=find(t>7,1)
plot(t(indStart:indEnd),roll_acel(indStart:indEnd))
xlabel('Time(s)')
ylabel('Acceleration roll axis(m/s/s)')
axis([-inf inf -13 -9])
legend('Simulation','Test Flight')
%%
t =flightdata(:,1);
indextEnd = length(t);
acc_x=flightdata(:,2);
