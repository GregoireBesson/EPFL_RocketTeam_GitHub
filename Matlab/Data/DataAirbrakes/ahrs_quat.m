function [q, K, g_bias] = ahrs_quat(q, K, g_bias, g_data, a_data, m_data, gravity_vector, mag_vector, dt)
% [q, K, g_bias] = ahrs_quat(q, K, g_bias, g_data, a_data, m_data, gravity_vector, mag_vector, dt)
...
%   K   - Kalman filter covarince matrix [6x6]
%   g_bias  - Gyroscopes biases vector [3x1]
%   g_data - Intergral of angular rate vector over the computer cycle [3x1]
%   a_data  - Acceleration vector in body frame [3x1]
%   m_data  - Magnetic field vector in body frame [3x1]
%   gravity_vector  - Gravity vector in navigation frame [3x1]
%   mag_vector  - Magetic field vector in navigation frame [3x1]
%   dt  - Computer cycle, sec.
%
%   Output arguments:
%   q  - Updated attitude quaternion [1x4]
%   P  - Updated Kalman filter covarince matrix [6x6]
%   g_bias - Updated gyroscopes biases vector [3x1]

%% Correct attitide increments  with estimated biases
g_data  = g_data-g_bias*dt;
...
fn_hat = quat_rot(q,a_data);
%"Magnetic field" vector estimate in navigation frame
mn_hat = quat_rot(q,m_data);

%Measurement vector
v = zeros(6,1);
v(1:3,1) = gravity_vector-fn_hat;
v(4:6,1) = mag_vector-mn_hat;
