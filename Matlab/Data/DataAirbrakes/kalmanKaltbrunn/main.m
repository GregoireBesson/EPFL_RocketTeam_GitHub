...
Q = [1 0 0; 0 1 0; 0 0 10];     % prédicrion du thrust mauvaise
...
for i = 1:length(z)-1
    ...
    % varalt = 0.0010 * 8436/(seaLevelhPA * ((1-x(1,i)/44330)^(5.255))^.8097); % prendre une constante pour le embeded, ça sert à rien d'être aussi précis. cette ligne est utile juste pour justifier qu'on prend une constante
    varalt = 0.0742;
...
hold on
plot(t, x_hat(1,:)','b','Linewidth',0.5,'DisplayName','Prediction');
plot(t, z(1,1:end-1)','g','Linewidth',0.5,'DisplayName','Measurment');
plot(t, x(1,1:end-1)','r--','Linewidth',1.5,'DisplayName','Correction');
hold off
ylabel('Altitude [m]')
yyaxis right
...
xlim([0 t(end)]);
%ylim([-20 260]);
title('Altitude Estimator');
legend show
%legend('Prediction','Measurment','Corection', '\sigma_{alt}')
grid on
set(gca,'fontsize', 16);
...
hold on
plot(t, x_hat(2,:)','b','Linewidth',0.5,'DisplayName','Prediction');
plot(t, z(2,1:end-1)','g','Linewidth',0.5,'DisplayName','Measurment');
plot(t, x(2,1:end-1)','r--','Linewidth',1.5,'DisplayName','Correction');
plot(t, x(2,1:end-1)'.*cosi(1:length(x(2,1:end-1))),'r','Linewidth',1.5,'DisplayName','Vertical Correction');
hold off
ylabel('Velocity [m/s]')
ylim([-10 70])
yyaxis right
...
hold on
plot(t, x_hat(3,:)','b','Linewidth',0.5,'DisplayName','Prediction');
plot(t, z(3,1:end-1)','g','Linewidth',0.5,'DisplayName','Measurment');
plot(t, x(3,1:end-1)','r--','Linewidth',1.5,'DisplayName','Correction');
hold off
